...
这里放你自己写的实现。**题目的完整说明在 [`projects/problems/stageN.md`](../problems/stageN.md)**，
...
│   ├── p1_1_statistics_test.cpp # 从 solutions/ 复制过来的测评程序
│   └── p1_1 (可执行文件，忽略)
├── stage2/
│   └── ...
└── ...
...
2. 把对应的测评文件从 `solutions/stageN/` 复制到同一个目录。
   原因：测评程序用 `#include "pX_Y_*.h"` 找你的头文件，而双引号包含会优先搜索
   **测评文件自己所在目录**；不复制的话，它会直接包含 `solutions/` 里的参考实现，你的代码根本没被编译。
3. 在该目录编译运行：
...
`-I../../solutions` 只是为了让测评文件找到 `test_util.h`，不需要其它参数。
...
for f in *_test.cpp; do
  base="${f%.cpp}"
  echo "=== $base ==="
  g++ -std=c++17 "$f" -I../../solutions -o "/tmp/$base" && "/tmp/$base"
done
```
...
先自己独立完成，再对照 `solutions/stageN/pX_Y_*.h`（参考实现，带详细注释）。
